(ns footwearops.operation
  "FootwearOperationActor -- one plant-operations coordination request =
  one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (FootwearAdvisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Footwear Plant
  ...
  (:require [langgraph.graph :as g]
            ...
            [footwearops.advisor :as advisor]
            [footwearops.governor :as governor]
...
(defn build
  "Compiles a FootwearOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `footwearops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; FootwearAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Footwear Plant Operations Governor -- independent censor.
      (g/add-node :govern
