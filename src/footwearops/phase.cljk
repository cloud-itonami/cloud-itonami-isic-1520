(ns footwearops.phase
  "Phase 0->3 staged rollout for the footwear plant-operations
  coordination actor.

    Phase 0  read-only          -- no writes, still governor-gated.
    Phase 1  assisted-intake    -- production-batch logging allowed,
                                    every write needs human approval.
    Phase 2  assisted-coordinate -- adds quality-concern flags and
  ...
  consequence (a cutting/sewing/molding maintenance window means
  production downtime, and the equipment ends up actually touched); it
  is always a human plant supervisor's call.
  `footwearops.governor`'s `line-operate-blocked-violations` HARD-blocks
  direct-operate attempts unconditionally, and the confidence/high-
  stakes gate independently never lets `:flag-quality-concern` auto-
...
(def phases
  ...
  {0 {:label "read-only"           :writes #{}                                            :auto #{}}
   ...
   2 {:label "assisted-coordinate" :writes #{:log-production-batch :flag-quality-concern
...
(defn verdict->disposition
  "Map a Footwear Plant Operations Governor verdict to a base
