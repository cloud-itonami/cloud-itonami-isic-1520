...
(deftest flag-quality-concern-never-auto-at-any-phase
  (doseq [[n {:keys [auto]}] phase/phases]
    (is (not (contains? auto :flag-quality-concern))
        (str "phase " n " must not auto-commit :flag-quality-concern"))))
...
(deftest gate-escalates-a-clean-non-auto-write
  ...
  (is (= :escalate (:disposition (phase/gate 3 {:op :flag-quality-concern} :commit))))
