(ns footwearops.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control cutting/lasting/assembly-line
  equipment directly... does NOT authorize or execute equipment
  operations') implemented faithfully. The single invariant under
  test:

    FootwearAdvisor never schedules maintenance, flags a quality
    concern, or coordinates a shipment the Footwear Plant Operations
    Governor would reject; `:schedule-maintenance`/`:flag-quality-
...
(deftest unknown-op-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t4" {:op :actuate-cutting-press :effect :propose :subject "x"} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
...
(deftest shipment-volume-exceeded-is-held-and-unoverridable
  (testing "a shipment proposal whose volume would exceed the batch's own logged volume -> HOLD"
    (let [[db actor] (fresh)
...
(deftest quality-concern-always-escalates-even-high-confidence
  (testing "flag-quality-concern always escalates -- never auto-committed, regardless of confidence"
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-quality-concern :effect :propose :subject "concern-1"
      ...
      (let [r2 (approve! actor "t12")]
        (is (= :commit (get-in r2 [:state :disposition])))
        (is (= 1 (count (store/quality-concerns db))))))))

(deftest quality-concern-approval-rejected-leaves-no-record-only-a-hold-fact
  (let [[db actor] (fresh)
        _ (exec-op actor "t13" {:op :flag-quality-concern :effect :propose :subject "concern-2"
                                :value {:equipment-id "equip-001" :concern-type :labor-safety
                                        :severity :low}}
                   coordinator)
        r (reject! actor "t13")]
    (is (= :hold (get-in r [:state :disposition])))
    (is (= 0 (count (store/quality-concerns db))) "rejected approval never reaches the commit node")
    (is (= 1 (count (store/ledger db))))))
